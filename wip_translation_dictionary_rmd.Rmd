...
# _ UDS 2 ----
if (GET_API_DATA) {
  data_dict_u2_json <- RCurl::postForm(
    uri=REDCAP_API_URI,
    token=REDCAP_API_TOKEN_UDS2,
    content='metadata',
    format='json',
...
# _ UDS 3 (Archive) ----
if (GET_API_DATA) {
  data_dict_u3a_json <- RCurl::postForm(
    uri=REDCAP_API_URI,
    token=REDCAP_API_TOKEN_UDS3a,
    content='metadata',
    format='json',
...
# _ UDS 3 (New) ----
if (GET_API_DATA) {
  data_dict_u3n_json <- RCurl::postForm(
    uri=REDCAP_API_URI,
    token=REDCAP_API_TOKEN_UDS3n,
    content='metadata',
    format='json',
...
# _ UDS 3 (Archive) ----
if (GET_API_DATA) {
  data_dict_ms_json <- RCurl::postForm(
    uri=REDCAP_API_URI,
    token=REDCAP_API_TOKEN_MINDSET,
    content='metadata',
    format='json',
...
Whatever export fields that aren't included in the standardized translation dictionary (`trans_dict_std`) need to be included in this standardized translation dictionary. Otherwise, these fields won't be included in the integrated dataset.
...
export_fields_data_dict_u2_df <-
  ...
  mutate(form_name = case_when(
    str_detect(export_field_name, "_complete$") ~ 
      str_replace(export_field_name, pattern = "_complete$", ""),
    TRUE ~ form_name
  ))
